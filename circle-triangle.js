function circletriangleMain() {
	...
	//document.getElementById("dbg").innerHTML = "Dbg: (" + 1 + "," + 2 + ")";

	//id = setInterval(doAngle, 40);
...
function onmousemove(e) {
	//document.getElementById("dbg").innerHTML += "Dbg: (" + e.clientX + "," + e.clientY + ")";
...
function update() {
	...
	if (coordsQ) drawCoords()
	if (quadQ) drawQuadrants()
	if (anglesQ) drawAngles()
}
...
function drawAngle() {
	...
	angle = angleSnap * Math.PI / 180.; // make the radian angle the same (tick, tick style)
	...
	if (angleSnap % 90 == 0) { // integer degrees can be at exact right angles, but radians rounded off will not be
		switch (angleSnap) {
			case 0:
				fs = [0, 1, 0];
				break;
...
function drawCoords() {
	...
	for (i = 0; i < angles.length; i++) {
		...
		// angles on circle
		if (i < angles.length - 1) { // not last angle (2pi)
			//document.getElementById("dbg").innerHTML += "drawCoords: (" + deg + "," + angle + ")";

			var angX = Math.cos(angle) * (circleRadius);
			var angY = -Math.sin(angle) * (circleRadius);
			//document.getElementById("dbg").innerHTML += "drawCoords: (" + angX + "," + angY + ")";
